...
class TagTable:
    def insert_new_tag(self, name: str, user_id: str, db: Session | None = None) -> TagModel | None:
    ...
    def get_tag_by_name_and_user_id(self, name: str, user_id: str, db: Session | None = None) -> TagModel | None:
        try:
            id = name.replace(' ', '_').lower()
            with get_db_context(db) as db:
    ...
    def get_tags_by_user_id(self, user_id: str, db: Session | None = None) -> list[TagModel]:
        with get_db_context(db) as db:
            return [TagModel.model_validate(tag) for tag in (db.query(Tag).filter_by(user_id=user_id).all())]

    def get_tags_by_ids_and_user_id(self, ids: list[str], user_id: str, db: Session | None = None) -> list[TagModel]:
        with get_db_context(db) as db:
            return [
    ...
    def delete_tag_by_name_and_user_id(self, name: str, user_id: str, db: Session | None = None) -> bool:
        try:
            with get_db_context(db) as db:
                id = name.replace(' ', '_').lower()
    ...
    def delete_tags_by_ids_and_user_id(self, ids: list[str], user_id: str, db: Session | None = None) -> bool:
    ...
    def ensure_tags_exist(self, names: list[str], user_id: str, db: Session | None = None) -> None:
