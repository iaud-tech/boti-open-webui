...
class ChatTable:
    ...
    def get_chats_by_model_id(self, model_id: str) -> list[ChatModel]:
        with get_db() as db:
            ...
            query = db.query(Chat).filter(Chat.chat['models'].contains([model_id])).order_by(Chat.updated_at.desc())

            all_chats = query.all()
            return [ChatModel.model_validate(chat) for chat in all_chats]

    def insert_new_chat(self, user_id: str, form_data: ChatForm) -> ChatModel | None:
        with get_db() as db:
            id = str(uuid.uuid4())
            chat = ChatModel(
                **{
